...
void EntityManager::update(Animal *animal, const int index, const float dt) {
	...
	// Plus proche proie
	addClosest(animal, entities[index].prays, inputs);
	
	// Plus proche prédateur
	addClosest(animal, entities[index].predators, inputs);
	
	animal->update(inputs, dt);
}

void EntityManager::addClosest(Animal *animal, const std::vector<int> &speciesIndexes, std::vector<float> &inputs) {
	Position closest(worldSize * worldSize);
	
	// find closest in all tabs
	for (int i = 0; i < speciesIndexes.size(); i++) {
		getClosestFromTab(animal->getPos(), entities[ speciesIndexes[i] ].tab, closest);
	}
	
	// Give closest's angle/dist to network 
	// angle as radians/2*PI and distance as sigmoid so both [0; 1[
	inputs.push_back(atan2f(closest.pos.y, closest.pos.x) / (2 * PI));
	inputs.push_back(1.f / (1.f + expf(-closest.dist/distanceSigmoid)) * 2.f - 1.f);
}

void EntityManager::getClosestFromTab(const Vect2i pos, const std::vector<Entity*> &tab, Position &closest) {
	Position tmp;
